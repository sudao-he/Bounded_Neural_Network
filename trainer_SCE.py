...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Fault Detection of ATC Antenna Beam -- SCE Loss Trainer")

    parser.add_argument("--version", type=int, default=12, help="model version")
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.1, help="alpha of SCE loss"
    )

    parser.add_argument(
        "--beta", type=float, default=1.0, help="beta of SCE loss"
    )
    ...
    log_dir = './logs/noisy_training/sce_loss/{}/run_{}/'.format(args.arch, args.version)
    ...
    criterion = SCELoss(alpha=args.alpha, beta=args.beta, num_classes=args.num_class)
    ...
    Save Model
    """
    save_path_model = './logs/noisy_training/sce_loss/{}/version_{}.pth'.format(args.arch, args.version)
    ...
    Save Logs
    """
    logger = get_logger('./logs/noisy_training/sce_loss/{}/model_log.log'.format(args.arch))
    logger.info('\n')
    args_dict = vars(args)
    ...
    """
    python -m tensorboard.main --logdir=./logs/noisy_training/sce_loss/BiGRU/run_12/ --port=6006
