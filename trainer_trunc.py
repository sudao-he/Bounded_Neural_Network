...
import os
from loss_lib import TruncatedLoss
import torch.nn as nn


def trainer_nn(model, device, optimizer, train_data, test_data, alpha, writer, epochs, early_stop, criterion, clip, version):
    step = 0
    best_acc = 0
    # try:
    for epoch in range(epochs):
        ...
        for i_batch, batch_data in enumerate(train_data):
            data, target, count = batch_data['data'], batch_data['noisy_labels'], batch_data['index']
            data = data.float()
            target = target.long()
            data = data.to(device)
            ...
            optimizer.zero_grad()
            output = model(data)
            loss = criterion(output, target, count)
            loss.backward()
        ...
        save_path_model = './logs/noisy_training/trunc_loss/BiGRU/version_{}_{}.pth'.format(version, epoch)
...
def eval_nll(data_iter, model, device, criterion):
    ...
    for i_batch, batch_data in enumerate(data_iter):
        feature, target, count = batch_data['data'], batch_data['label'], batch_data['index']
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--noise_ratio", type=float, default=0.2, help="noise ratio of label noise"
    )
    ...
    parser.add_argument(
        "--q", type=float, default=0.7, help="q of truncated loss"
    )

    parser.add_argument(
        "--k", type=float, default=0.5, help="k of truncated loss"
    )
    ...
    log_dir = './logs/train_ratio//{}/run_{}/'.format(args.arch, args.version)
    ...
    Save Logs
    """
    save_path_model = './logs/train_ratio/{}/version_{}.pth'.format(args.arch, args.version)
    logger = get_logger('./logs/train_ratio//{}/model_log.log'.format(args.arch))
    logger.info('\n')
    args_dict = vars(args)
    ...
    criterion = TruncatedLoss(trainset_size=len(dataset), q=args.q, k=args.k).to(device)
    model_star = trainer_nn(model=model, device=device, optimizer=optimizer, train_data=train_dataloader,
                            ...
                            early_stop=args.early_stop, criterion=criterion, clip=args.clip, version=args.version)
    ...
    """
    python -m tensorboard.main --logdir=./logs/train_ratio/BiGRU/run_3/ --port=6006
